...
'BBVA-P-423 RQADM-12 GVARGAS 14/02/2017 Carrusel Telefonos
...
'BUG-PD-244: CGARCIA: 20/10/2017: ACTUALIZA EMAIL DE CANCELACION
...
Partial Class aspx_BaseNegativa
    ...
    Dim usu As String
    Private Sub aspx_BaseNegativa_Load(sender As Object, e As EventArgs) Handles Me.Load
    ...
    Private Sub btnprocesar_Click(sender As Object, e As EventArgs) Handles btnprocesar.Click
        ...
        Dim hitCarrusel As Boolean = CarruselTelefono(Request("Sol").ToString())
        ...
        If (hitCarrusel) Then
            ...
            'Return
            'Master.MensajeError("RECHAZO Solicitud no viable por políticas.")
                ...
                If (CInt(dtConsulta.Tables(0).Rows(0).Item("RESULTADO").ToString) = 1) Then
                    If (CInt(dtConsulta.Tables(1).Rows(0).Item("PDK_STATUS").ToString) = 2) Then
                        If (CInt(dtConsulta.Tables(1).Rows(0).Item("PDK_STATUS_EMAIL").ToString) = 2 Or CInt(CInt(dtConsulta.Tables(1).Rows(0).Item("PDK_STATUS_SMS").ToString) = 2)) Then
                            ...
                            strLocation = ("../aspx/ValidaEmails.aspx?idPantalla=69&Sol=" & Val(Request("Sol")).ToString & "&mostrarPant=2" & "&usuario=" & usu)
        ...
        Else
            ...
            If muestrapant = 0 Then
                ...
                Dim path As String = "../aspx/" & dslink.Tables(0).Rows(0).Item("PDK_PANT_LINK").ToString() & "?idPantalla=" & dslink.Tables(0).Rows(0).Item("PDK_ID_PANTALLAS").ToString() & "&sol=" & Val(Request("Sol")).ToString() & "&usuario=" & Val(Request("usuario")).ToString()
    ...
    Private Function CarruselTelefono(ByVal folio As String) As Boolean
        ...
        Try
            cmd.CommandText = "get_HIT_BNTELEFONO_SP"
