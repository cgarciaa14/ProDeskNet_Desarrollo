'BBV-P-423: BUG-PD-62: ERODRIGUEZ: 01/06/2017 Funcionalidad caja de notas-->
'BUG-PD-85 : ERODRIGUEZ 10/06/2017 :  se modifico index del grid
...
Partial Class aspx_CajadeNotas
        ...
        If (lista_solicitudes <> "") Then
            ...
            If (paramIsNumber And opc = 1) Then
                ...
                Try
                    ...
                    '    Dim usuarioborra As String = "SELECT C.PDK_FN_STATUS_PESTANA FROM PDK_REL_USU_PER A INNER JOIN PDK_PERMISO_CARATULA B ON A.PDK_ID_PERFIL = B.PDK_ID_PERFIL INNER JOIN PDK_PERMISO_CARATULA_PESTANA C ON B.PDK_FI_ID_PERMISO_CARATULA_PESTANA = C.PDK_FI_ID_PERMISO_CARATULA_PESTANA WHERE A.PDK_CLAVE_USUARIO = " + Session("IdUsua").ToString() + " AND C.PDK_FC_NOMBRE_PESTANA = 'botonBorrar'"
                    '    tmp = BD.EjecutarQuery(usuarioborra)
                    '    If tmp.Tables(0).Rows.Count > 0 Then

                    '        Dim borra As Boolean = Convert.ToBoolean(tmp.Tables(0).Rows(0).Item(0))

                    '        gvMensajes.Columns(5).Visible = borra
                    '        gvMensajes.Columns(6).Visible = borra

                    Dim pconsulta As String = "select id id_nota, * from PDK_CAJA_NOTAS WHERE estatus = 1 and PDK_CAJA_NOTAS.PDK_ID_SOLICITUD in(" + lista_solicitudes + ")"
                    tmp = BD.EjecutarQuery(pconsulta)

                    If tmp.Tables(0).Rows.Count > 0 Then
                        'El usuario tiene mensajes en la caja de notas
                        pconsulta = "select A.id id_nota, A.fe_creacion, A.fe_borrado, A.mensaje, A.estatus, A.usu_creacion, A.usu_borrado, A.PDK_ID_SOLICITUD, A.leido , A.Tarea ,B.PDK_USU_NOMBRE +' '+ B.PDK_USU_APE_PAT +' '+ B.PDK_USU_APE_MAT as Usuario from PDK_CAJA_NOTAS A INNER JOIN PDK_USUARIO B on B.PDK_ID_USUARIO = A.usu_creacion  WHERE estatus = 1  and A.PDK_ID_SOLICITUD in(" + lista_solicitudes + ") AND A.PDK_ID_SOLICITUD = " + Convert.ToString(sol) + "  ORDER BY fe_creacion DESC"
                        tmp = BD.EjecutarQuery(pconsulta)
                        If tmp.Tables(0).Rows.Count > 0 Then
                    ...
                    'Master.MensajeError("No cuentas con permisos suficientes")
                    '        End If
                Catch ex As Exception
                    Master.MensajeError(ex.Message)
                    textEditor.Visible = True
